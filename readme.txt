...
* Can be used with the SecurImage plugin to provide a captcha solution for guest users
...
I always assumed that the standard use case would require logins, but I always have had the option of allowing
guest users to give feedback. In the event you are getting spam from using an open form like this, you would most
likely want a captcha solution to minimize it. For my ease of coding, I integrated the [SecurImage](https://wordpress.org/plugins/securimage-wp/) plugin when using the guest form.
...
= 1.4.0 =
* Added captchas via SecureImage. You must have that plugin installed and have "Allow unregistered users" set to yes for the captcha to appear.
...
= 1.4.0 =
* New feature release. Upgrade recommended, but not required.
